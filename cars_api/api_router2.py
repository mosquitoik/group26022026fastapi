...
from cars_api.storage2 import storage
...
def get_cars(
    ...
    saved_cars = storage.get_cars(q=q, page=page)
...
def patch_car(car_id: str, new_car_data: CarPatchSchema) -> CarSavedSchema:
    patched_car = storage.patch_car(car_id, new_car_data)
...
def put_car(car_id: str, car: CarCreateSchema) -> CarSavedSchema:
    put_car_obj = storage.put_car(car_id, car)
